...
class Mazmorra:
    ...
    def jugar(self):
        ...
        #abrimos un bucle que va a durar mientras siga habiendo
        #enemigos en la mazmorra
        while dead_monsters < number_of_monsters:
            ...
            if not self.monster.esta_vivo():
                #si muere el monstruo buscamos un tesoro y aumentamos
                 # en 1 el contador de enemigos muertos
                self.encontrar_tesoro()
                dead_monsters += 1
            else:
                #si muere el heroe mostramos mensaje y ponemos el
                # contador de enemigos muertos en un valor fuera
                # del rango posible
                print("Héroe ha sido derrotado en la mazmorra.")
                dead_monsters = 10

        #comprobamos si el valor de enemigos muertos es un valor
        # dentro del rango de enemigos posibles, si no es asi
        # es que el que murió fue el heroe
        if dead_monsters < 6:
            print("¡" + self.hero.nameHero + " ha derrotado a todos los "
                  + "monstruos y ha conquistado la mazmorra!")
