...
def aficiones():
    #controlar que checkButtons estan activos y los guarda
    if read_var.get():
        read = " leer"
    else:
        read = ""
...
#crear checkButtons
check = tk.Checkbutton(root,text="Leer", variable=read_var,
                       command=aficiones)
check.pack()

check2 = tk.Checkbutton(root,text="Música", variable=music_var,
                        command=aficiones)
check2.pack()

check3 = tk.Checkbutton(root,text="Deporte", variable=sport_var,
                        command=aficiones)
check3.pack()
